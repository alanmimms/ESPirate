...
/* ESPirate Xtensa window-safe setjmp / longjmp to bypass ROM syscall */
int espirate_setjmp(void *env);
__attribute__((noreturn)) void espirate_longjmp(void *env, int val);
#define LUAI_THROW(L,c)     espirate_longjmp((c)->b, 1)
#define LUAI_TRY(L,c,f,ud)  if (espirate_setjmp((c)->b) == 0) ((f)(L, ud))
